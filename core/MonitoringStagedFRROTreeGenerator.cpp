...
/*
 * MonitoringStagedFRROTreeGenerator.cpp
 *
 *  Created on: Mar 31, 2017
 *      Author: Rodrigo Schmidt/Luiza Carneiro Pizzi
 */

#include "MonitoringStagedFRROTreeGenerator.h"
...
MonitoringStagedFRROTreeGenerator::MonitoringStagedFRROTreeGenerator(
...
MonitoringStagedFRROTreeGenerator::MonitoringStagedFRROTreeGenerator(
		...
		vector<AbstractConstraintFunction<double,int> *>gam, vector<AbstractConstraintFunction<double,int> *>epsLim, vector<AbstractConstraintFunction<double,int> *>nu, AlternativeAdimSproutingVolumetricCostEstimator *localEstimator) {
...
AbstractObjectCCOTree *MonitoringStagedFRROTreeGenerator::generate(long long int saveInterval, string tempDirectory) {
...
#pragma omp critical
			...
			if (minCost < INFINITY) {
				...
				localEstimator->previousState(tree, minParent, xNew, minBif, dLim);
				vector<double> contributions = localEstimator->computeContributionsCost(tree);
				cout << "Values of the contributions: " << contributions[0] << " "<< contributions[1] <<" "<< contributions[2] << endl;
...
AbstractObjectCCOTree *MonitoringStagedFRROTreeGenerator::generateExperimental(long long int saveInterval, string tempDirectory, int maxNumOfTrials) {
...
int MonitoringStagedFRROTreeGenerator::isValidRootSegment(point xNew,
		int iTry) {
...
int MonitoringStagedFRROTreeGenerator::isValidSegment(point xNew, int iTry) {
...
StagedDomain * MonitoringStagedFRROTreeGenerator::getDomain() {
	return domain;
}

vector<AbstractObjectCCOTree *> MonitoringStagedFRROTreeGenerator::getTrees() {
	vector<AbstractObjectCCOTree *> trees;
	trees.push_back(tree);
	return trees;
}

void MonitoringStagedFRROTreeGenerator::enableConfigurationFile(
		string filename) {
	this->isGeneratingConfFile = 1;
	this->confFilename = filename;
}

void MonitoringStagedFRROTreeGenerator::generatesConfigurationFile(ios::openmode mode) {
...
void MonitoringStagedFRROTreeGenerator::markTimestampOnConfigurationFile(
		string label) {
	if (isGeneratingConfFile) {
...
void MonitoringStagedFRROTreeGenerator::closeConfigurationFile() {
...
AbstractObjectCCOTree *MonitoringStagedFRROTreeGenerator::resume(long long int saveInterval, string tempDirectory) {
...
AbstractObjectCCOTree *MonitoringStagedFRROTreeGenerator::resumeExperimental(long long int saveInterval, string tempDirectory, int maxNumOfTrials) {
...
AbstractObjectCCOTree *MonitoringStagedFRROTreeGenerator::resumeSavePoints(long long int saveInterval, string tempDirectory, FILE *fp) {
	this->beginTime = time(nullptr);
	this->dLimInitial = this->dLim;
...
AbstractObjectCCOTree *MonitoringStagedFRROTreeGenerator::resumeSavePointsMidPoint(long long int saveInterval, string tempDirectory, FILE *fp) {
	this->beginTime = time(nullptr);
	this->dLimInitial = this->dLim;
...
void MonitoringStagedFRROTreeGenerator::observableModified(IDomainObservable* observableInstance) {
...
AbstractObjectCCOTree*& MonitoringStagedFRROTreeGenerator::getTree() {
	return tree;
}

void MonitoringStagedFRROTreeGenerator::setSavingTasks(const vector<AbstractSavingTask*>& savingTasks){
	this->savingTasks = savingTasks;
}

void MonitoringStagedFRROTreeGenerator::saveStatus(long long int terminals){
	tree->setPointCounter(domain->getPointCounter());
...
vector<AbstractConstraintFunction<double, int> *>* MonitoringStagedFRROTreeGenerator::getGams()
{
	return &(this->gams);
}

vector<AbstractConstraintFunction<double, int> *>* MonitoringStagedFRROTreeGenerator::getEpsLims()
{
	return &(this->epsLims);
}
vector<AbstractConstraintFunction<double, int> *>* MonitoringStagedFRROTreeGenerator::getNus()
{
	return &(this->nus);
}

double MonitoringStagedFRROTreeGenerator::getDLim() {
	return this->dLim;
}

void MonitoringStagedFRROTreeGenerator::setDLim(double newDLim) {
	this->dLim = newDLim;
}

double MonitoringStagedFRROTreeGenerator::getDLimInitial() {
	return this->dLimInitial;
}

double MonitoringStagedFRROTreeGenerator::getDLimLast() {
	return this->dLimLast;
}

time_t MonitoringStagedFRROTreeGenerator::getBeginTime() {
	return this->beginTime;
}

time_t MonitoringStagedFRROTreeGenerator::getEndTime() {
	return this->endTime;
}
